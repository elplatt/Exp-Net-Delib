...
class TestLearning(unittest.TestCase):
    ...
    def test_get_random_belief(self):
        p = [0.5, 0.5, 0.6, 0.6, 0.3, 0.3]
        with mock.patch('learning.np.random.uniform', mock_uniform):
            belief = learning.get_random_belief(p)
        self.assertEqual(
            belief,
            [0, 1, 0, 1, 0, 1]
        )
    ...
    def test_initial_beliefs(self):
        ...
        p1 = [0.5, 0.5, 0.6, 0.6, 0.3, 0.3]
        with mock.patch('learning.np.random.uniform', mock_uniform):
            belief = learning.initial_beliefs(G, p1)
        self.assertEqual(
            belief,
            [0, 1, 0, 1, 0, 1])
